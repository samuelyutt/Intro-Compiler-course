...
FunctionNode::FunctionNode(int _line_number, int _col_number,
                           string _function_name,
                           vector<NodeWithTypeList *> *_parameters,
                           VariableInfo *_return_type, Node _body,
                           int _end_line_number, int _end_col_number,
                           string _end_name,
    ...
    for (uint i = 0; i < this->prototype.size(); i++) {
        ...
        case SET_ACCUMLATED:
            ...
            for (uint j = 0; j < this->prototype[i]->array_range.size(); j++) {
                std::cout << "[";
                std::cout << to_string(
                    this->prototype[i]->array_range[j].end -
                    this->prototype[i]->array_range[j].start);
                std::cout << "]";
            }
            break;
        default:
